...
@section('content_header')
    <div class="p-1 flex-grow-1">
        <h1 class="m-0 text-dark">Edit movie</h1>
    </div>
@stop
            ...
            <form action="{{route('movies.update', $movie)}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="form-group col-md-6">
                        ...
                        <input type="text" name="title" class="form-control" id="title" placeholder="Title" value="{{old('title') ?? $movie->title}}" aria-describedby="titleHelp">
                        @error('title')
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input type="text" name="year" class="form-control" id="year" placeholder="Year"
                               value="{{old('year') ?? $movie->year}}" aria-describedby="yearHelp">
                        @error('year')
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" class="form-control" id="description" aria-describedby="descriptionHelp">{{old('description') ?? $movie->description}}</textarea>
                    @error('description')
                ...
                <div class="form-group">
                    ...
                    <select name="genres[]" id="genres" class="form-control" multiple="multiple" aria-describedby="genresHelp">
                        @foreach($genres as $genre)
                            <option value="{{$genre->id}}" @if($movie->genres()->find($genre->id)) selected @endif>{{$genre->name}}</option>
                        @endforeach
                    </select>
                    @error('genres')
                ...
                <div class="form-group">
                    ...
                    <select name="countries[]" id="countries" class="form-control" multiple="multiple" aria-describedby="countriesHelp">
                        @foreach($countries as $country)
                            <option value="{{$country->id}}" @if($movie->countries()->find($country->id)) selected @endif>{{$country->name}}</option>
                        @endforeach
                    </select>
                    @error('countries')
